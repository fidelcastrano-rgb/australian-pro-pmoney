...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1. Persist the Order to Supabase PostgreSQL using Prisma if database is connected
    let savedOrder = null;
    if (isDatabaseConfigured()) {
      try {
        ...
        console.warn("Database Save Notice:", dbError instanceof Error ? dbError.message : dbError);
      }
    } else {
      console.log(`[Database Notice] DATABASE_URL is not set or using placeholder. Order ${orderNumber} processed cleanly.`);
    }

    // 2. Dispatch Emails via Zoho SMTP
    const emailFrom = process.env.EMAIL_FROM || "sales@australianpropmoney.com.au";
    
    // Parse admin email address(es) into a clean array
    const rawAdmin = process.env.EMAIL_ADMIN || "sales@australianpropmoney.com.au, order@australianpropmoney.com.au";
    ...
    const custResult = await sendEmail({
      ...
      to: email,
      replyTo: adminEmails[0] || "sales@australianpropmoney.com.au",
    ...
    if (custResult.success) {
      console.log(`Order review email sent to customer: ${email}`);
    } else {
      console.warn(`Customer order email status: ${custResult.error}`);
    }
    ...
    if (adminResult.success) {
      console.log(`Admin order alert sent to: ${adminEmails.join(", ")}`);
    } else {
      console.warn(`Admin order alert status: ${adminResult.error}`);
    }
